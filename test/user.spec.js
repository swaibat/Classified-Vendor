...
describe('user registration', () => {
  ...
  it('create user account', (done) => {
    chai
      .request(app)
      .post(
        `/api/v1/users/register/${AuthHelper.createToken('rswaib@gmail.com')}`
      )
      .send(Data.user1)
      .end((err, res) => {
  ...
  it('check if user exists', (done) => {
    chai
      .request(app)
      .post(
        `/api/v1/users/register/${AuthHelper.createToken('rswaib2@gmail.com')}`
      )
      .send(Data.user1)
      .end((err, res) => {
        res.should.have.status(409);
...
describe('user signin', () => {
  ...
  it('invalid login details', (done) => {
    chai
      .request(app)
      .post('/api/v1/users/login')
      .send({ username: 'hfhfh', password: 'helccccz' })
      .end((err, res) => {
        res.should.have.status(400);
        res.body.should.be.a('object');
        res.body.should.have.property('message').eql('Invalid login details');
  ...
  it('invalid login details', (done) => {
    chai
      .request(app)
      .post('/api/v1/users/login')
      .send({ username: 'hfhfh', password: 'hel' })
      .end((err, res) => {
        res.should.have.status(400);
        res.body.should.be.a('object');
        res.body.should.have.property('message').eql('password should be greater than 3');
        done();
      });
  });
});
